...
// Object to store timeouts for each chat
const gameTimeouts = {};

const resetInactivityTimer = (chat, ww) => {
    // Clear existing timeout if any
    if (gameTimeouts[chat]) {
        clearTimeout(gameTimeouts[chat]);
    }
    
    // Set new timeout for 3 hours (3 * 60 * 60 * 1000 milliseconds)
    gameTimeouts[chat] = setTimeout(() => {
        if (ww[chat]) {
            delete ww[chat];
            console.log(`Deleted werewolf game session for chat ${chat} due to inactivity`);
        }
    }, 3 * 60 * 60 * 1000);
};
