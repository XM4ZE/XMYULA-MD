...
async function bindv2(conn, targetId) {
	...
	const privateChats = Object.keys(conn.chats).filter(
		id => id.endsWith('@s.whatsapp.net') || id.endsWith(global.xmyulamd)
	);
...
`.trim()
		...
		decodeLid: {
			async value(conn, idsc, chat) {
				return await decodeNumber(conn, idsc, chat)
			},
			enumerable: true
		},
		adReply: {
...
export async function smsg(conn, m, hasParent) {
	...
	if (m.message?.[m.mtype]?.contextInfo?.mentionedJid) {
		...
		const convertedJids = await Promise.all(mentionedJids.map(async jid => {
			if (jid.includes(global.xmyulamd) && m.chat?.endsWith('@g.us')) {
				try {
					const converted = await decodeNumber(conn, jid, m.chat);
					return converted.includes(global.xmyulamd) ? jid : converted;
				} catch (e) {
	...
	if (participant.includes(global.xmyulamd) && groupJid.endsWith('@g.us')) {
		try {
	...
	if (m.message?.[m.mtype]?.contextInfo?.quotedMessage) {
		try {
			...
			if (participant?.includes(global.xmyulamd) && m.chat?.endsWith('@g.us')) {
...
export function serialize() {
	...
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		sender: {
			get() {
				return this._sender || this.key?.participant || this.participant || this.chat || ''
			},
			enumerable: true
		},
		fromMe: {
...
async function decodeNumber(conn, idsc, groupJid) {
	try {
		if (!idsc.includes(global.xmyulamd) || !groupJid.endsWith('@g.us')) {
			return idsc
		}
